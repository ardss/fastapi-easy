...
class FastAPIEasy(FastAPI):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if migration_mode not in valid_modes:
            raise ValueError(
                f"migration_mode 必须是 {valid_modes} 之一，" f"收到: {migration_mode}"
            )
    ...
    async def _startup(self):
        ...
        try:
            ...
            if self.models:
                for model in self.models:
                    if not hasattr(model, "__table__"):
                        raise TypeError(f"Invalid model: {model} - missing __table__ attribute")
            ...
            # 创建迁移引擎
            self._migration_engine = MigrationEngine(engine, metadata, mode=self.migration_mode)
