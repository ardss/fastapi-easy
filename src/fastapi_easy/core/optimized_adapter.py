    ...
    async def get_all(
        ...
        try:
            result = await self._execute_with_timeout(
                self.base_adapter.get_all(filters, sorts, pagination), "get_all"
            )
        except asyncio.TimeoutError:
        ...
        for attempt in range(max_retries):
            if await self.lock_manager.acquire(lock_key):
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(
                        f"Failed to acquire lock for get_one({id}) after {max_retries} attempts"
                    )
                    return None
        ...
        for attempt in range(max_retries):
            if await self.lock_manager.acquire(lock_key):
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(
                        f"Failed to acquire lock for update({id}) after {max_retries} attempts"
                    )
    ...
    async def warmup_cache(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug(
                    f"Cache warmup attempt {attempt + 1}/{max_retries}: loading up to {limit} items"
                )
