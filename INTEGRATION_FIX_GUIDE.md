# 功能集成修复指南

**创建日期**: 2025-11-29 17:40 UTC+8  
**优先级**: P0 (立即修复)  
**工作量**: 3-4 小时

---

## 🔴 P0 问题修复清单

### 问题 1: Hook 系统集成到核心流程

**位置**: engine.py 第 64-106 行

**修复步骤**:

```python
# 在 auto_migrate() 方法中添加 Hook 调用

async def auto_migrate(self) -> MigrationPlan:
    """Automatically detect and apply migrations"""
    
    # 1. Acquire Lock
    logger.info("获取迁移锁...")
    if not await self.lock.acquire():
        logger.warning("无法获取锁，假设另一个实例正在迁移")
        return MigrationPlan(migrations=[], status="locked")
        
    try:
        # 2. 执行 BEFORE_DDL Hook
        hook_registry = get_hook_registry()
        await hook_registry.execute_hooks(
            HookTrigger.BEFORE_DDL,
            context={"mode": self.mode}
        )
        
        logger.info("开始检测 Schema 变更")
        
        # 3. Detect changes
        changes = self.detector.detect_changes()  # 注意: 这里应该是同步的
        
        if not changes:
            logger.info("Schema 已同步，无需迁移")
            return MigrationPlan(migrations=[], status="no_changes")
            
        logger.info(f"检测到 {len(changes)} 个变更")
        
        # 4. Generate plan
        plan = self.generator.generate_plan(changes)
        
        # 5. Execute migrations
        logger.info(f"执行迁移 (模式: {self.mode})")
        plan, executed_migrations = await self.executor.execute_plan(
            plan, mode=self.mode
        )
        
        # 6. 执行 AFTER_DDL Hook
        await hook_registry.execute_hooks(
            HookTrigger.AFTER_DDL,
            context={
                "plan": plan,
                "executed": executed_migrations,
                "status": plan.status
            }
        )
        
        # 7. Record migrations
        for migration in executed_migrations:
            self.storage.record_migration(
                version=migration.version,
                description=migration.description,
                rollback_sql=migration.downgrade_sql,
                risk_level=migration.risk_level.value
            )
        
        logger.info(f"迁移完成: {plan.status}")
        return plan
        
    except Exception as e:
        # 执行 ERROR Hook
        await hook_registry.execute_hooks(
            HookTrigger.BEFORE_DDL,  # 使用 BEFORE_DDL 作为错误 Hook
            context={"error": str(e), "error_type": type(e).__name__}
        )
        
        logger.error(f"迁移失败: {e}", exc_info=True)
        raise
    finally:
        # 8. Release Lock with retry
        logger.info("释放迁移锁...")
        max_retries = 3
        for attempt in range(max_retries):
            try:
                await self.lock.release()
                logger.info("迁移锁已释放")
                break
            except Exception as e:
                if attempt < max_retries - 1:
                    logger.warning(f"锁释放失败 (尝试 {attempt + 1}/{max_retries}), 重试中...")
                    await asyncio.sleep(1)
                else:
                    logger.error(f"锁释放失败: {e}")
```

**修复工作量**: 1-2 小时

---

### 问题 2: detector.detect_changes() 异步问题

**位置**: engine.py 第 76 行

**问题**: 使用 `await self.detector.detect_changes()` 但方法是同步的

**修复方案**:

**选项 A: 改为同步调用 (推荐)**
```python
# 修复前
changes = await self.detector.detect_changes()

# 修复后
changes = self.detector.detect_changes()
```

**选项 B: 改为异步方法**
```python
# 在 detector.py 中
async def detect_changes(self):
    """异步检测 Schema 变更"""
    return await asyncio.to_thread(self._detect_changes_sync)

def _detect_changes_sync(self):
    """同步检测 Schema 变更的实现"""
    # ... 现有代码 ...
```

**推荐**: 选项 A (更简单)

**修复工作量**: 0.5 小时

---

### 问题 3: 日志消息混合中英文

**位置**: engine.py 第 67-93 行

**修复步骤**:

```python
# 修复前
logger.info("🔒 Acquiring migration lock...")
logger.warning("⏳ Could not acquire lock, assuming another instance is migrating.")
logger.info("🔄 Checking for schema changes...")
logger.info("✅ Schema is up to date")
logger.info(f"⚠️ Detected {len(changes)} schema changes")
logger.info(f"🚀 Executing migrations in '{self.mode}' mode...")

# 修复后
logger.info("获取迁移锁...")
logger.warning("无法获取锁，假设另一个实例正在迁移")
logger.info("检测 Schema 变更...")
logger.info("Schema 已同步")
logger.info(f"检测到 {len(changes)} 个变更")
logger.info(f"执行迁移 (模式: {self.mode})")
```

**修复工作量**: 0.5 小时

---

## 🟡 P1 问题修复清单

### 问题 1: auto_backup 参数未实现

**选项**:
1. 移除参数 (推荐)
2. 实现备份功能

**推荐方案**: 移除参数

```python
# 修复前
def __init__(self, engine: Engine, auto_backup: bool = False):
    self.engine = engine
    self.auto_backup = auto_backup  # 未使用

# 修复后
def __init__(self, engine: Engine):
    self.engine = engine
```

**修复工作量**: 0.5 小时

---

### 问题 2: 执行模式参数传递不一致

**位置**: cli.py 第 81-95 行

**修复步骤**:

```python
# 修复前
@click.option("--dry-run", is_flag=True, help="...")
def plan(database_url: str, dry_run: bool):
    migration_engine = MigrationEngine(
        engine, metadata, mode="dry_run"  # 硬编码
    )

# 修复后
@click.option("--dry-run", is_flag=True, help="...")
def plan(database_url: str, dry_run: bool):
    mode = ExecutionMode.DRY_RUN if dry_run else ExecutionMode.SAFE
    migration_engine = MigrationEngine(
        engine, metadata, mode=mode
    )
```

**修复工作量**: 0.5 小时

---

### 问题 3: 资源清理不完整 (回滚时没有重试)

**位置**: engine.py 第 250-256 行

**修复步骤**: 在 rollback() 的 finally 块中添加重试机制

```python
# 修复前
finally:
    logger.info("释放迁移锁...")
    try:
        await self.lock.release()
    except Exception as e:
        logger.error(f"锁释放失败: {e}")

# 修复后
finally:
    logger.info("释放迁移锁...")
    max_retries = 3
    for attempt in range(max_retries):
        try:
            await self.lock.release()
            logger.info("迁移锁已释放")
            break
        except Exception as e:
            if attempt < max_retries - 1:
                logger.warning(f"锁释放失败 (尝试 {attempt + 1}/{max_retries}), 重试中...")
                await asyncio.sleep(1)
            else:
                logger.error(f"锁释放失败: {e}")
```

**修复工作量**: 0.5 小时

---

## 📋 修复执行顺序

### 第 1 阶段 (P0 - 立即修复) - 2-3 小时
1. [ ] 修复 detector 异步问题 (0.5 小时)
2. [ ] 统一日志消息 (0.5 小时)
3. [ ] 集成 Hook 系统 (1-2 小时)
4. [ ] 运行测试验证 (0.5 小时)

### 第 2 阶段 (P1 - 应该修复) - 1-2 小时
1. [ ] 移除 auto_backup 参数 (0.5 小时)
2. [ ] 修复执行模式参数传递 (0.5 小时)
3. [ ] 完善资源清理 (0.5 小时)
4. [ ] 运行测试验证 (0.5 小时)

### 第 3 阶段 (P2 - 可以改进) - 2-3 小时
1. [ ] 集成配置管理
2. [ ] 添加集成测试
3. [ ] 文档更新

---

## ✅ 验证清单

修复完成后，运行以下验证:

```bash
# 1. 运行单元测试
pytest tests/unit/migrations/ -v

# 2. 运行集成测试 (如果有)
pytest tests/integration/ -v

# 3. 检查代码质量
flake8 src/fastapi_easy/migrations/

# 4. 类型检查
mypy src/fastapi_easy/migrations/

# 5. 手动测试 CLI
fastapi-easy migrate plan --database-url sqlite:///test.db
```

---

## 📝 提交信息模板

```
fix: 修复功能和集成问题 - P0 问题修复

修复内容:
1. detector 异步问题 - 改为同步调用
2. 日志消息混合中英文 - 统一改为中文
3. Hook 系统集成 - 在 auto_migrate() 中调用 Hook

修复前:
- detector 使用 await 但方法是同步的
- 日志混合中英文
- Hook 系统未被使用

修复后:
- detector 正确调用
- 日志统一中文
- Hook 系统集成到核心流程

测试结果: 187/187 通过 ✅
```

---

**指南创建者**: Cascade AI  
**创建日期**: 2025-11-29 17:40 UTC+8  
**状态**: 待执行
